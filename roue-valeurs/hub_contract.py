...
ALLOWED_SCOPES = {'ROUE_VALEURS_RUN','ROUE_VALEURS_STATUS','ROUE_VALEURS_DOCUMENT_READ'}
MAX_TOKEN = 8192
...
def verify_launch_token(token, secret, now=None):
    ...
    if not isinstance(scopes,list) or 'ROUE_VALEURS_RUN' not in scopes or any(x not in ALLOWED_SCOPES for x in scopes): raise ValidationError('Droits Hub invalides.')
