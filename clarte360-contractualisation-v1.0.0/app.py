...
from xlsm_safe import inspect_workbook, first_available_conv_row, patch_conv_adm, replace_financements_for_action, force_recalc_on_open, action_row, workbook_values
...
def calc_finance_rows(no_clar, rows):
    ...
    for i,r in enumerate(rows, start=1):
        ...
        out.append({
            'ID_FINANCEMENT':f'FIN-{no_clar}-{i:02d}','NO_CLAR':no_clar,'ORDRE':i,
            ...
            'CODE_POST_FINANCEUR':r.get('CODE_POST_FINANCEUR',''),'VILLE_FINANCEUR':r.get('VILLE_FINANCEUR',''),
            'EMAIL_FACTURATION':r.get('EMAIL_FACTURATION',''),'MONTANT_HT':round(ht,2),'TAUX_TVA':rate,
...
with c3:
    birth=st.date_input('Date de naissance',value=to_date(b.get('date_naissance'),date(1990,1,1)))
...
c1,c2,c3=st.columns(3)
with c1:
    duree=st.number_input('Durée d’accompagnement (heures)',min_value=0.5,value=13.0,step=0.5)
    nb_temps=st.number_input('Nombre de temps / séances',min_value=1,value=9,step=1)
with c2:
    default_start=date(2026,9,3) if aps else date.today()
    default_end=date(2026,11,14) if aps else date.today()
    date_debut=st.date_input('Début de l’action',value=default_start)
    date_fin=st.date_input('Fin de l’action',value=default_end)
with c3:
    modalite=st.text_input('Lieu / modalité',value='Bilan de compétences ONLINE – Teams' if prestation=='Bilan de compétences' else str(mod.get('format_souhaite','')))
    consultant=st.text_input('Accompagnateur',value='BRIET Dominique')
    consultant_email=st.text_input('E-mail accompagnateur',value='dbriet@clarte360.com')
consultant_tel=st.text_input('Téléphone accompagnateur',value='0663822302')

calendar_default='03/09/2026 – Phase préliminaire – 1 h\n19/09/2026 – Séance 1 – 1 h 30\n03/10/2026 – Séance 2 – 1 h 30\n10/10/2026 – Séance 3 – 1 h 30\n17/10/2026 – Séance 4 – 1 h 30\n24/10/2026 – Séance 5 – 1 h 30\n31/10/2026 – Séance 6 – 1 h 30\n07/11/2026 – Séance 7 – 1 h 30\n14/11/2026 – Séance 8 / conclusion – 1 h 30'
calendrier=st.text_area('Planning prévisionnel',value=calendar_default if aps else str(existing.get('CALENDRIER','') if mode.startswith('Reprendre') else ''),height=220)
...
c1,c2,c3=st.columns(3)
with c1: total_ttc=st.number_input('Prix total TTC',min_value=0.0,value=1560.0,step=10.0)
with c2: taux_tva=st.number_input('TVA (%)',min_value=0.0,value=20.0,step=1.0)
with c3: st.metric('Prix HT calculé',f"{total_ttc/(1+taux_tva/100):,.2f} €".replace(',', ' ').replace('.', ','))

seed=[
    {'TYPE_FINANCEUR':'BENEFICIAIRE','NOM_FINANCEUR':f'{prenom} {nom}'.strip(),'MONTANT_TTC':150.0,'TAUX_TVA':taux_tva,'FACTURE_A_ETABLIR_A':f'{prenom} {nom} {adresse} {cp} {ville}'.strip(),'STATUT_FINANCEMENT':'Prévu','OBSERVATIONS':'Participation personnelle'},
    {'TYPE_FINANCEUR':'AUTRE FINANCEUR','NOM_FINANCEUR':'ADCA – R&D','MONTANT_TTC':1410.0,'TAUX_TVA':taux_tva,'FACTURE_A_ETABLIR_A':'ADCA – R&D','STATUT_FINANCEMENT':'Prévu','OBSERVATIONS':'Prise en charge R&D'}]
if 'fin_editor' not in st.session_state: st.session_state.fin_editor=seed
...
diff=round(total_ttc-sum_fin,2)
if abs(diff)<0.01: st.success(f'Financement équilibré : {sum_fin:,.2f} € TTC'.replace(',', ' ').replace('.', ','))
else: st.error(f'Écart de financement : {diff:,.2f} €'.replace(',', ' ').replace('.', ','))
...
if prestation=='Bilan de compétences' and contract_type=='Particulier – bipartite' and date_debut < contract_date:
...
can_generate=abs(diff)<0.01 and bool(prenom and nom and email and total_ttc>0)
...
if can_generate:
    if st.button('7. Préparer la base mise à jour et le contrat PDF',type='primary',use_container_width=True):
        ...
        contract_data={'beneficiaire':{'civilite':b.get('civilite',''),'prenom':prenom,'nom':nom,'date_naissance':birth,'adresse':adresse,'code_postal':cp,'ville':ville,'email':email,'telephone':telephone},
                       'action':{'no_clar':no_clar,'date_contrat':contract_date,'duree_heures':duree,'date_debut':date_debut,'date_fin':date_fin,'modalite':modalite,'calendrier':calendrier,'demande':demande,'objectifs':objectifs,'criteres_reussite':criteres,'suivi_6_mois':'Un entretien de suivi est proposé environ six mois après la conclusion du bilan. Il est distinct des 13 heures d’accompagnement contractualisées.'},
