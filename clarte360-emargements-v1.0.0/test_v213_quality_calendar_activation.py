...
def test_quality_uses_real_last_session_not_administrative_end_date():
    ...
    assert cold.strftime('%Y-%m-%d %H:%M')=='2027-02-17 20:30'



def test_quality_specific_cold_date_keeps_real_end_clock():
    e=eng(); aid=base_action(e,planned=1.5)
    add_slot(e,aid,'2026-11-19','19:00','20:30','test',send=-90)
    execute(e,"UPDATE actions SET quality_cold_due_date='2027-03-31' WHERE id=:a",{'a':aid})
    cold=local_dt(standard_quality_due(e,aid,'COLD'),'Europe/Paris')
    assert cold.strftime('%Y-%m-%d %H:%M')=='2027-03-31 20:30'

def test_quality_fallback_without_calendar_is_end_of_admin_day_not_noon():
    e=eng(); aid=base_action(e,planned=0)
    hot=local_dt(standard_quality_due(e,aid,'HOT'),'Europe/Paris')
    assert hot.strftime('%Y-%m-%d %H:%M')=='2026-12-31 23:59'
