...
def validate_bank(bank: dict) -> list[str]:
    ...
    if len(items)!=120: errors.append(f"La banque doit contenir 120 items, reçu {len(items)}.")
    if len(ids)!=len(set(ids)): errors.append("Les IDs item ne sont pas uniques.")
    dc=Counter(x.get("dimension") for x in items)
    if set(dc)!=set(DIMS) or any(dc[d]!=20 for d in DIMS): errors.append(f"Répartition dimensions invalide: {dict(dc)}")
    fc=Counter(x.get("facette") for x in items)
    if set(fc)!=FACETS or any(fc[f]!=4 for f in FACETS): errors.append("Les 30 facettes doivent comporter exactement 4 items.")
