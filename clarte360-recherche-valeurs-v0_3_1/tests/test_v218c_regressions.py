...
def test_8c_version_and_removed_useless_accompanist_review():
    assert 'APP_VERSION = "2.1.3.8c-preproduction"' in TEXT
...
def test_module2_shows_all_questions_and_hydrates_legacy_answers():
    ...
    assert "for index,q in enumerate(MODULE2_QUESTIONS,1)" in TEXT
    assert "Vos réponses sont enregistrées. Elles restent visibles" in TEXT
